...
import { ArrowLeft, Check, Crown, Loader2, Sparkles, Zap, ShieldCheck } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="container mx-auto px-4 lg:px-8 py-12 max-w-6xl">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">Pick the plan that fits your workflow. Upgrade or cancel anytime. All plans include access to YAIDEV's AI Powered Builder.</p>
        </motion.div>
            ...
            {plans.map((p, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-1">
                    <span className="font-heading font-bold text-4xl text-foreground">${(p.price_cents / 100).toFixed(0)}</span>
                    <span className="text-muted-foreground text-sm"> / month</span>
                  </div>
                  <p className="text-sm text-primary font-semibold mb-5">{p.monthly_credits.toLocaleString()} credits / month</p>
                  ...
                  >
                    ...
                    {paying === p.slug ? "Redirecting…" : `Subscribe to ${p.name}`}
                  </button>
                </motion.div>
              );
            })}
        ...
        <div className="mt-10 flex items-center justify-center gap-2 text-xs text-muted-foreground">
          ...
          Secure checkout via Paystack. Cards, bank transfer, mobile money supported.
        </div>
      </section>
    </div>
  );
